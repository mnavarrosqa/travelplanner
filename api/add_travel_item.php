...
try {
    ...
    $sql = "
        ...
        (trip_id, type, title, description, notes, start_datetime, end_datetime, location, confirmation_number, created_by";
    
    $values = "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?";
    $params = [$tripId, $type, $title, $description ?: null, $notes, $startDatetime, $endDatetime ?: null, $location ?: null, $confirmationNumber ?: null, $userId];
    
    if ($type === 'hotel' && $hotelData) {
        $sql .= ", hotel_data";
        $values .= ", ?";
        $params[] = $hotelData;
    }
